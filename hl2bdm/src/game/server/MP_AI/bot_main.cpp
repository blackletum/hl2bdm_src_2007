...
CON_COMMAND_F( hl2bdm_bot_add, "Add a bot.", 0 /*FCVAR_CHEAT*/ )
{
...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t *pEdict, const char *playername )
	{
		...
		CHL2MP_Player::s_PlayerEdict = pEdict;
...
CBasePlayer *BotPutInServer( bool  bFrozen, int iTeam )
{
	...
	char botname[ 64 ];
	Q_snprintf( botname, sizeof( botname ), "HL2BDMBot%02i", g_CurBotNumber );
	...
	pBot->m_flSkill[BOT_SKILL_YAW_RATE] = random->RandomFloat(SKILL_MIN_YAW_RATE, SKILL_MAX_YAW_RATE);
	pBot->m_flSkill[BOT_SKILL_SPEED] = random->RandomFloat(SKILL_MIN_SPEED, SKILL_MAX_SPEED);
	pBot->m_flSkill[BOT_SKILL_STRAFE] = random->RandomFloat( SKILL_MIN_STRAFE, SKILL_MAX_STRAFE);
...
void Bot_Think( CSDKBot *pBot )
{
	...
	//I will switch my weapon.
	if ( pBot->GetPlayerModelType() == PLAYER_SOUNDS_METROPOLICE || pBot->GetPlayerModelType() == PLAYER_SOUNDS_COMBINESOLDIER )
	{
		pBot->Weapon_Switch( pBot->Weapon_OwnsThisType( "weapon_stunstick" ) );
	}
	else if ( pBot->GetPlayerModelType() == PLAYER_SOUNDS_CITIZEN )
	{
